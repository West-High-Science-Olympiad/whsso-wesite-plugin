...
if (!function_exists('sticky_anything_config_page')) {
	...
	<div id="sticky-anything-settings-general" class="wrap">
		...
		<p><?php _e('Pick any element on your page, and it will stick when it reaches the top of the page when you scroll down. Usually handy for navigation menus, but can be used for any (unique) element on your page. If multiple unique elements are specified, they will stack vertically (for example, a tabbed page can be made to have sticky tabs immediately below the sticky menu).'); ?></p>
		<div class="main-content">

			<?php
				if ( isset( $_GET['sticky-anything-tab'] )) {
					$activeTab = $_GET['sticky-anything-tab'];
				} else {
					$activeTab = 'main';
				}
        if ($activeTab != 'main' && $activeTab != 'advanced' && $activeTab != 'faq') {
          $activeTab = 'main';
			...
			<h2 id="nav-tab-wrapper-sticky-anything">
				<button class="nav-tab <?php if ($activeTab == 'main') { echo 'nav-tab-active2'; } ?>" href="#main"><?php echo 'Basic settings'; ?></button>
				<button class="nav-tab <?php if ($activeTab == 'advanced') { echo 'nav-tab-active2'; } ?>" href="#advanced"><?php echo 'Advanced settings'; ?></button>
				<button class="nav-tab <?php if ($activeTab == 'faq') { echo 'nav-tab-active2'; } ?>" href="#faq"><?php echo 'Information'; ?></button>
			</h2>
			...
			<div class="tabs-content tab">
				...
				<form method="post" action="admin-post.php">
					...
					<div class="sticky-anything-tab-content tab-sticky-main <?php if ($activeTab != 'main') {echo 'hide';} ?>">
					...
					<div class="sticky-anything-tab-content tab-sticky-advanced <?php if ($activeTab != 'advanced') {echo 'hide';}?>">
					...
					<div class="sticky-anything-tab-content tab-sticky-main tab-sticky-advanced <?php if (($activeTab != 'main') && ($activeTab != 'advanced')) {echo 'hide';} ?>">
				...
				<div class="sticky-anything-tab-content tab-sticky-faq <?php if ($activeTab != 'faq') {echo 'hide';} ?>">
	...
	function process_sticky_anything_options() {
		...
		update_option( 'sticky_anything_options', $options );
 		wp_redirect( add_query_arg(
 			array('page' => 'whsso-plugin', 'message' => '1', 'tab' => 'sticky', 'sticky-anything-tab' => $tabValue),
